...
import { PDFPrintServiceFactory, printPdf } from "web-print_service";
...
appConfig: null,
  ...
  async run(config) {
    ...
    // #2374 modified by ngx-extended-pdf-viewer
...
function closeSecondaryToolbar({ target }) {
  ...
  if (
    ...
    if (
      target &&
      target.parentElement === secondaryToolbar.toggleButton
    ) {
      return;
    }
    if (
